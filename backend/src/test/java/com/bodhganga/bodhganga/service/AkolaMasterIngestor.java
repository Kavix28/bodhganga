...
public class AkolaMasterIngestor {
    ...
    void performSafeAkolaMasterIngestion() throws Exception {
        ...
        List<Question> existingAkolaBefore = questionRepo.findByStateSlugAndDistrictSlug("maharashtra", "akola");
        ...
        if (!matchingHash.isEmpty()) {
            System.out.println(
                    "Clearing " + matchingHash.size() + " stale hash-matched questions for clean master ingestion");
            questionRepo.deleteAll(matchingHash);
        }

        MockMultipartFile qMultipart = new MockMultipartFile(
                "questionPdf", "2-Final MCQs Question bank Akola District.pdf", "application/pdf", qBytes);
        MockMultipartFile aMultipart = new MockMultipartFile(
                "answerPdf", "8-MCQs Solution with explanation Akola District.pdf", "application/pdf", aBytes);
        ...
        long publishedActiveMaster = allAkolaAfter.stream()
                .filter(q -> result.getFileHash().equals(q.getFileHash()) && "PUBLISHED".equalsIgnoreCase(q.getStatus())
        ...
        long missingText = newlyIngested.stream().filter(q -> q.getQuestion() == null || q.getQuestion().isBlank())
                .count();
        long missingOptions = newlyIngested.stream().filter(q -> q.getOptions() == null || q.getOptions().size() < 4)
                .count();
