...
public class RealEastJaintiaPdfIngestionTest {
    ...
    private File findEastJaintiaQuestionFile() {
        File f = new File("C:\\Users\\chris\\OneDrive\\Desktop\\Bodhganga\\2- Final East Jaintia Hills Advanced MCQ Question Bank.pdf");
        if (f.exists()) return f;
        f = new File("C:\\Users\\chris\\Desktop\\Bodhganga\\2- Final East Jaintia Hills Advanced MCQ Question Bank.pdf");
        if (f.exists()) return f;
        return null;
    }

    private File findEastJaintiaAnswerFile() {
        File f = new File("C:\\Users\\chris\\OneDrive\\Desktop\\Bodhganga\\16-MCQs Solution with explanation East Jaintia Hills District.pdf");
        if (f.exists()) return f;
        f = new File("C:\\Users\\chris\\Desktop\\Bodhganga\\16-MCQs Solution with explanation East Jaintia Hills District.pdf");
        if (f.exists()) return f;
        return null;
    }
    ...
    void testRealEastJaintiaPdfEndToEndIngestionPipeline() throws Exception {
        ...
        System.out.println("==================================================");
        System.out.println("Question PDF File: " + qFile.getName());
        System.out.println("Question PDF Path: " + qFile.getAbsolutePath());
        System.out.println("Question PDF Size: " + qFile.length() + " bytes");
        ...
        System.out.println("--------------------------------------------------");
        System.out.println("Answer PDF File: " + aFile.getName());
        System.out.println("Answer PDF Path: " + aFile.getAbsolutePath());
        System.out.println("Answer PDF Size: " + aFile.length() + " bytes");
        ...
        for (QuestionParserService.ParsedQuestion pq : parsedQs) {
            if (pq.getQuestionNumber() <= 0 || pq.getQuestionText() == null || pq.getQuestionText().isBlank() || pq.getOptions() == null || pq.getOptions().size() < 4) {
                malformedCount++;
            }
        ...
        Map<Integer, Question> qMap = ingestedQs.stream().collect(Collectors.toMap(Question::getQuestionNumber, q -> q, (e1, e2) -> e1));
        Map<Integer, DeterministicQuestionClassifier.QuestionClassification> classificationMap = new HashMap<>();

        for (int i = 1; i <= 80; i++) {
            Question q = qMap.get(i);
            if (q != null) {
                DeterministicQuestionClassifier.QuestionClassification cls = deterministicQuestionClassifier.classifyQuestion(q.getQuestion(), q.getOptions(), q.getLevel());
                classificationMap.put(i, cls);
                if (cls.getClassification() == DeterministicQuestionClassifier.ClassificationResult.FOUNDATION) {
                    foundationCount++;
                } else if (cls.getClassification() == DeterministicQuestionClassifier.ClassificationResult.STATEMENT_BASED) {
                    statementCount++;
                } else {
                    reviewRequiredCount++;
                }
        ...
        System.out.println("--------------------------------------------------------------------------------------------------");
        System.out.printf("%-4s | %-20s | %-13s | %-10s | %-20s | %s%n", "Q#", "FINAL CLASSIFICATION", "SOURCE", "CONFIDENCE", "MATCHED SIGNALS", "REASON");
        System.out.println("--------------------------------------------------------------------------------------------------");

        for (int i = 1; i <= 80; i++) {
            Question q = qMap.get(i);
            if (q == null) {
                System.out.printf("%-4d | %-20s | %-13s | %-10s | %-20s | %s%n", i, "REVIEW_REQUIRED", "N/A", "LOW", "none", "Question missing from PDF parser");
                continue;
            }
            ...
            if (signals.isBlank()) signals = "direct-factual";
        ...
        for (int i = 1; i <= 80; i++) {
            Question q = qMap.get(i);
            if (q != null && ("REVIEW_REQUIRED".equalsIgnoreCase(q.getStatus()) || q.getIsActive() == null || q.getIsActive())) {
                rrCount++;
                DeterministicQuestionClassifier.QuestionClassification cls = deterministicQuestionClassifier.classifyQuestion(q.getQuestion(), q.getOptions(), q.getLevel());
                ...
                System.out.println("Why Ambiguous: " + (q.getExplanation() == null ? "Missing solution explanation or ambiguous statement pattern" : "Requires manual audit"));
                System.out.println("--------------------------------------------------");
            }
        }
        if (rrCount == 0) {
        ...
        for (Question q : ingestedQs) {
            ...
            assertTrue("DRAFT".equalsIgnoreCase(q.getStatus()) || "REVIEW_REQUIRED".equalsIgnoreCase(q.getStatus()),
                    "Question Q" + q.getQuestionNumber() + " status must be DRAFT or REVIEW_REQUIRED but was: " + q.getStatus());
        }
    }
}
